...
class GoalSeekTest < Minitest::Test
  ...
  def test_same
    assert_equal 0, (GoalSeeker.seek  :start=>0 , :goal=>0, :function=>lambda { |x| x }, seeker_type: :brute_force)
  end

  def test_next_step
    assert_equal 1, (GoalSeeker.seek  start: 0 , goal: 1, function: lambda { |x| x }, seeker_type: :brute_force)
    assert_equal 10, (GoalSeeker.seek  start: 0 , goal: 10, function: lambda { |x| x }, seeker_type: :brute_force)
  end

  def test_param_is_not_goal
    assert_equal 5, (GoalSeeker.seek  start: 0 , goal: 10, function: lambda { |x| 2*x }, seeker_type: :brute_force)
  end

  def test_stop_after_a_while
    assert 100 >= (GoalSeeker.seek  start: 0 , goal: 1000, max_cycles:100,
      function: lambda { |x| 2*x }, seeker_type: :brute_force)
  end

  def test_functions_with_multiple_answers
    assert_equal 3, (GoalSeeker.seek  start: 100 , goal: 0, max_cycles:100,
      function: lambda { |x| (x * x - 5 * x + 6) }, seeker_type: :brute_force) # answers can be 2 or 3
  end

  def test_step_in_the_oposite_direction
    assert_equal -1, (GoalSeeker.seek  start: 0 , goal: -1, max_cycles:10, function: lambda { |x| x }, seeker_type: :brute_force)
  end
